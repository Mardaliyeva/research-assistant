...
def get_llm() -> LLMProvider:
    ...
    # Imports stay inside branches so unused SDKs are not required at import time.
...
def get_vlm() -> VLMProvider:
    ...
    provider = os.getenv("LLM_PROVIDER", "anthropic").lower().strip()
    # VLM selection reuses LLM_PROVIDER because the same vendor/model family is used.
    if provider == "anthropic":
...
def get_embedder() -> EmbeddingProvider:
    ...
    # Embeddings are separate because not every LLM vendor exposes embedding models.
    if provider == "openai":
