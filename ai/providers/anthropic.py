...
class _AnthropicMixin:
    ...
    def _setup(self, model: str | None, api_key: str | None) -> None:
        # Shared setup avoids duplicating API-key and SDK initialization logic.
...
class AnthropicLLM(_AnthropicMixin, LLMProvider):
    ...
    def complete(
        ...
        *,
        ...
        full_prompt = prompt
        # Anthropic receives the schema as instruction text when JSON is requested.
...
class AnthropicVLM(_AnthropicMixin, VLMProvider):
    ...
    def describe(
        ...
        *,
        ...
        # Claude vision uses base64 image blocks in the Messages API.
...
def _guess_media_type(path: Path) -> str:
    # Tell the API the exact MIME type for the base64 image payload.
