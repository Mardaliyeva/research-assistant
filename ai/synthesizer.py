...
# The prompt forces evidence-grounded answers and bracketed citation markers.
...
def _format_source_block(sources: list[Source]) -> str:
    # Numbering starts at 1 because users/readers expect citations like [1].
    lines: list[str] = []
...
# Matches citation groups such as [1] and [1, 2].
...
def _extract_cited_indices(answer: str, n_sources: int) -> list[int]:
    ...
    """
    used: set[int] = set()
    # Use a set to dedupe repeated citations before returning sorted indices.
...
def synthesize(
    ...
    *,
    ...
    # Allow tests to inject a fake LLM; otherwise use the configured provider.
    llm = llm or get_llm()
    ...
    # Only include sources the model actually cited; invalid indices are filtered out.
