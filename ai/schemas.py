...
class Source(BaseModel):
    ...
    # extra='forbid' catches malformed provider responses during validation.
    ...
    def _title_nonempty(cls, v: str) -> str:
        # Empty titles make references hard to read, so reject them early.
        if not v.strip():
...
class AnswerWithCitations(BaseModel):
    ...
    # The answer itself is mutable so the SE layer can sanitize returned text.
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        # which is what the SE layer and the demo renderer expect.
        return {
