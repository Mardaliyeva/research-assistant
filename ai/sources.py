...
def _require_httpx():
    # Delay the import until a live fetch is requested so offline tests stay lightweight.
    try:
...
async def fetch_wikipedia(
    ...
    *,
    ...
    # If no shared client is supplied, create and later close a temporary one.
    if own_client:
    ...
    try:
        ...
        for title in titles:
            ...
            body = summ.json()
            # Skip disambiguation/empty pages that have no useful extract.
...
async def fetch_arxiv(
    ...
    *,
    ...
    own_client = client is None
    # Reuse a caller-owned AsyncClient when orchestrator supplies one.
    if own_client:
        client = httpx.AsyncClient(timeout=timeout)
    try:
        try:
...
def _parse_arxiv_atom(xml_text: str) -> list[Source]:
    ...
    out: list[Source] = []
    # Atom entries are namespaced, so every tag lookup uses the Atom namespace prefix.
    for entry in root.findall(f"{_ATOM_NS}entry"):
        ...
        if title_el is None or summary_el is None or id_el is None:
            continue
        # Normalize XML whitespace so abstracts render cleanly in prompts and CLI output.
...
class TavilyProvider(WebSearchProvider):
    ...
    async def search(
        ...
        *,
        ...
        httpx = _require_httpx()
        # Provider methods also support dependency-injected clients for testing/reuse.
        own = client is None
        if own:
        ...
        try:
            ...
            body = r.json()
            # Tavily returns results with title/url/content fields.
            return [
                Source(
...
class SerperProvider(WebSearchProvider):
    ...
    async def search(
        ...
        *,
        ...
        httpx = _require_httpx()
        # Serper needs a JSON POST with the API key in the X-API-KEY header.
        own = client is None
        if own:
            client = httpx.AsyncClient(timeout=self._timeout)
        try:
            ...
            body = r.json()
            # Serper's organic results use `link` instead of `url`.
            return [
                Source(
                    title=item.get("title", "(untitled)"),
                    url=item.get("link", ""),
    ...
    async def search(
        ...
        *,
        ...
        def _run() -> list[Source]:
            # The DuckDuckGo package is synchronous, so this work runs in a thread.
            results: list[Source] = []
            with DDGS() as ddgs:
...
def get_web_search_provider() -> WebSearchProvider:
    ...
    # Default to Tavily because it has a simple API and predictable response shape.
    if name == "tavily":
...
async def fetch_web(
    ...
    *,
    ...
    if not query.strip():
        return []
    # Tests can pass a fake provider; production resolves one from the environment.
